...
- [x] `npm install class-validator class-transformer -w apps/api`
- [x] `npm install class-validator class-transformer -w packages/shared`
- [x] В `apps/api/src/main.ts` подключить `ValidationPipe` глобально:
  ```ts
...
- [x] Создать файл `apps/api/src/auth/decorators/current-user.decorator.ts`
...
- [x] `apps/api/src/user/queries/get-user-by-id.query.ts` — класс с `userId: string`
- [x] `apps/api/src/user/queries/get-user-by-id.handler.ts` — `@QueryHandler(GetUserByIdQuery)`, инжектит `PrismaService`, возвращает `prisma.user.findUnique({ where: { id } })`
- [x] Зарегистрировать `GetUserByIdHandler` в `providers` файла `apps/api/src/user/user.module.ts`

### 6. CategoriesModule — структура
- [x] Создать директорию `apps/api/src/categories/`
...
- [x] Создать `apps/api/src/categories/categories.controller.ts`
...
### 8. CategoriesModule
- [x] Создать `apps/api/src/categories/categories.module.ts`
...
### 9. AppModule
- [x] В `apps/api/src/app.module.ts` добавить `CategoriesModule` в `imports`
...
- [x] `npm run dev:api` и прогон через curl:
...
- `apps/api/src/main.ts` — глобальный `ValidationPipe`
- `apps/api/src/auth/decorators/current-user.decorator.ts` — новый
- `apps/api/src/user/queries/get-user-by-id.{query,handler}.ts` — новые
- `apps/api/src/user/user.module.ts` — регистрация нового handler
- `apps/api/src/categories/**` — весь новый модуль
- `apps/api/src/app.module.ts` — подключение `CategoriesModule`
